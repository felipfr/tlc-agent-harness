...
import { existsSync as existsSync20, mkdirSync as mkdirSync14, readFileSync as readFileSync20, realpathSync, rmSync as rmSync4, writeFileSync as writeFileSync13 } from "node:fs";
...
function modeFilePath(root) {
  return join21(projectStateDir(root), "harness-mode");
}
function grindFlagPath(root) {
  return join21(flagsDir(root), "grind-on");
}
function skipFlagPath(root) {
  return join21(flagsDir(root), "skip-verify");
}
function focusFlagPath(root) {
  return join21(flagsDir(root), "focus");
}
function pairedFlagPath(root) {
  return join21(flagsDir(root), "paired");
}
function ensureFlagsDir(root) {
  mkdirSync14(flagsDir(root), { recursive: true });
}
...
function gatesPaused(root) {
  return existsSync20(skipFlagPath(root));
}
...
function setMode(root, raw) {
  ...
  ensureFlagsDir(root);
  writeFileSync13(modeFilePath(root), `${mode}
`);
  coreFacade.policy.refreshPolicyBaselines(root);
...
function resolveExecutable(name, env = process.env, platform = process.platform) {
  ...
  if (name.includes("/") || name.includes("\\")) {
    return candidates(name).find((candidate) => existsSync20(candidate)) ?? null;
  }
  for (const dir of (env.PATH ?? "").split(delimiter)) {
    if (!dir) {
      continue;
    }
    const found = candidates(join21(dir, name)).find((candidate) => existsSync20(candidate));
    if (found) {
      return found;
    }
  }
...
function setGateCommand(root, field, argv, interactive) {
  ...
  const parsed = existsSync20(path) ? JSON.parse(readFileSync20(path, "utf8")) : {};
  const grind = { ...parsed.grind ?? {} };
  grind[field === "test" ? "testCommand" : "lintCommand"] = argv;
  parsed.grind = grind;
  mkdirSync14(join21(root, ".tlc", "harness"), { recursive: true });
  writeFileSync13(path, `${JSON.stringify(parsed, null, 2)}
`, "utf8");
  coreFacade.policy.refreshPolicyBaselines(root);
...
function execBinPath() {
  return join21(resolveHarnessRoot(), "bin", "tlc-exec");
}
function buildBinPath() {
  return join21(resolveHarnessRoot(), "bin", "tlc-build");
}
function route(args) {
...
function runUpdate(root) {
  ...
  if (!existsSync20(join21(dest, "bin", "tlc-exec.mjs"))) {
  ...
  if (existsSync20(join21(dest, ".git"))) {
  ...
  const binDir = process.env.TLC_BIN_DIR || join21(homedir3(), ".local", "bin");
  mkdirSync14(binDir, { recursive: true });
  mkdirSync14(join21(home, "..", "skills"), { recursive: true });
  if (process.platform === "win32") {
    const installPs1 = join21(dest, "install.ps1");
    ...
    const tlcBin = join21(dest, "bin", "tlc");
    const skillSrc = join21(dest, "skills", "harness-init");
    const skillDest = join21(home, "..", "skills", "harness-init");
    spawnSync("ln", ["-sfn", tlcBin, join21(binDir, "tlc")], { stdio: "inherit" });
    if (!existsSync20(skillSrc)) {
    ...
    const hooks = spawnSync(process.execPath, [join21(dest, "bin", "write-user-hooks.mjs")], {
      stdio: "inherit",
      env: { ...process.env, TLC_HOME: home }
    });
    if ((hooks.status ?? 1) !== 0) {
