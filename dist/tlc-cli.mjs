...
import { existsSync as existsSync17, mkdirSync as mkdirSync12, readFileSync as readFileSync18, realpathSync, rmSync as rmSync3, writeFileSync as writeFileSync11 } from "node:fs";
...
function modeFilePath(root) {
  return join18(projectStateDir(root), "harness-mode");
}
function grindFlagPath(root) {
  return join18(flagsDir(root), "grind-on");
}
function skipFlagPath(root) {
  return join18(flagsDir(root), "skip-verify");
}
function headsDownFlagPath(root) {
  return join18(flagsDir(root), "heads-down");
}
function pairedFlagPath(root) {
  return join18(flagsDir(root), "paired");
}
function ensureFlagsDir(root) {
  mkdirSync12(flagsDir(root), { recursive: true });
}
function readMode(root) {
  const modeFile = modeFilePath(root);
  if (existsSync17(modeFile)) {
  ...
  if (existsSync17(headsDownFlagPath(root))) {
    return "focus";
  }
  if (existsSync17(pairedFlagPath(root))) {
    return "paired";
  }
  return "solo";
}
function grindOn(root) {
  return existsSync17(grindFlagPath(root)) || readMode(root) === "focus";
}
function gatesPaused(root) {
  return existsSync17(skipFlagPath(root));
}
...
function setMode(root, raw) {
  ...
  ensureFlagsDir(root);
  writeFileSync11(modeFilePath(root), `${mapped}
`);
...
function execBinPath() {
  return join18(resolveHarnessRoot(), "bin", "tlc-exec");
}
function buildBinPath() {
  return join18(resolveHarnessRoot(), "bin", "tlc-build");
}
function route(args) {
...
function runUpdate(root) {
  ...
  if (!existsSync17(join18(dest, "bin", "tlc-exec.mjs"))) {
  ...
  if (existsSync17(join18(dest, ".git"))) {
  ...
  const binDir = process.env.TLC_BIN_DIR || join18(homedir3(), ".local", "bin");
  mkdirSync12(binDir, { recursive: true });
  mkdirSync12(join18(home, "..", "skills"), { recursive: true });
  if (process.platform === "win32") {
    const installPs1 = join18(dest, "install.ps1");
    ...
    const tlcBin = join18(dest, "bin", "tlc");
    const skillSrc = join18(dest, "skills", "harness-init");
    const skillDest = join18(home, "..", "skills", "harness-init");
    spawnSync("ln", ["-sfn", tlcBin, join18(binDir, "tlc")], { stdio: "inherit" });
    if (!existsSync17(skillSrc)) {
    ...
    const hooks = spawnSync(process.execPath, [join18(dest, "bin", "write-user-hooks.mjs")], {
      stdio: "inherit",
      env: { ...process.env, TLC_HOME: home }
    });
    if ((hooks.status ?? 1) !== 0) {
