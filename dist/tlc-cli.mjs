...
import { existsSync as existsSync24, mkdirSync as mkdirSync15, readFileSync as readFileSync24, realpathSync, rmSync as rmSync4, writeFileSync as writeFileSync14 } from "node:fs";
...
function gatesPaused(root) {
  return existsSync24(skipFlagPath(root));
}
...
function resolveExecutable(name, env = process.env, platform = process.platform) {
  ...
  if (name.includes("/") || name.includes("\\")) {
    return candidates(name).find((candidate) => existsSync24(candidate)) ?? null;
  }
  for (const dir of (env.PATH ?? "").split(delimiter)) {
    if (!dir) {
      continue;
    }
    const found = candidates(join25(dir, name)).find((candidate) => existsSync24(candidate));
    if (found) {
      return found;
    }
  }
...
function setGateCommand(root, field, argv, interactive) {
  ...
  const parsed = existsSync24(path) ? JSON.parse(readFileSync24(path, "utf8")) : {};
...
function runUpdate(root) {
  ...
  if (!existsSync24(join25(dest, "bin", "tlc-exec.mjs"))) {
