...
// tools/obs-cli.ts
import { existsSync as existsSync21, mkdirSync as mkdirSync15, readdirSync as readdirSync5, writeFileSync as writeFileSync14 } from "node:fs";
...
function latestSessionId(root) {
  const sessions = join23(projectStateDir(root), "sessions");
  if (!existsSync21(sessions)) {
    return null;
  }
...
function main(argv) {
  ...
  if (cmd === "report") {
    ...
    const reportsDir = join23(projectStateDir(root), "reports");
    mkdirSync15(reportsDir, { recursive: true });
    const path = join23(reportsDir, `${conversationId}.md`);
    writeFileSync14(path, markdown);
    if (json) {
