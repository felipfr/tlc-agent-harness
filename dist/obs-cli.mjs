...
// tools/obs-cli.ts
import { existsSync as existsSync19, mkdirSync as mkdirSync14, readdirSync as readdirSync5, writeFileSync as writeFileSync13 } from "node:fs";
...
function latestSessionId(root) {
  const sessions = join20(projectStateDir(root), "sessions");
  if (!existsSync19(sessions)) {
    return null;
  }
...
function main(argv) {
  ...
  if (cmd === "report") {
    ...
    const reportsDir = join20(projectStateDir(root), "reports");
    mkdirSync14(reportsDir, { recursive: true });
    const path = join20(reportsDir, `${conversationId}.md`);
