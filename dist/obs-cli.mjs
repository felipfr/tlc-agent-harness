...
// tools/obs-cli.ts
import { existsSync as existsSync20, mkdirSync as mkdirSync14, readdirSync as readdirSync5, writeFileSync as writeFileSync13 } from "node:fs";
...
function latestSessionId(root) {
  const sessions = join21(projectStateDir(root), "sessions");
  if (!existsSync20(sessions)) {
    return null;
  }
  const last = readdirSync5(sessions).filter((name) => name.endsWith(".json")).sort().at(-1);
...
function main(argv) {
  ...
  if (cmd === "report") {
    ...
    const reportsDir = join21(projectStateDir(root), "reports");
    mkdirSync14(reportsDir, { recursive: true });
    const path = join21(reportsDir, `${conversationId}.md`);
