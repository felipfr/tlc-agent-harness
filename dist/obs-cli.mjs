// tools/obs-cli.ts
import { existsSync as existsSync17, mkdirSync as mkdirSync12, readdirSync as readdirSync4, writeFileSync as writeFileSync11 } from "node:fs";
...
var cmd = (process.argv[2] ?? "live").toLowerCase();
...
if (cmd === "report") {
  ...
  if (!conversationId) {
    const sessions = join18(projectStateDir(root), "sessions");
    if (!existsSync17(sessions)) {
      console.error("no sessions yet");
      process.exit(1);
    }
    const files = readdirSync4(sessions).filter((f) => f.endsWith(".json")).sort();
  ...
  const reportsDir = join18(projectStateDir(root), "reports");
  mkdirSync12(reportsDir, { recursive: true });
  const path = join18(reportsDir, `${conversationId}.md`);
  writeFileSync11(path, markdown);
