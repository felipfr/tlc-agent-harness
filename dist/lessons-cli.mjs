...
async function main(argv) {
  ...
  if (cmd === "add") {
    ...
    const refs = flagValues(rest, "--ref").map((raw) => coreFacade.lesson.parseLessonLink(raw)).filter((ref) => ref !== null);
    ...
    if (until !== undefined && Date.parse(until) <= Date.now()) {
      ...
      console.error("Such a lesson is never injected and the next garden prunes it. Use a future date, or omit --until.");
      process.exit(1);
    }
    ...
    if (json) {
      ...
      if (saved.refs.length > 0) {
        console.log(`  refs: ${saved.refs.map((ref) => coreFacade.lesson.formatLessonLink(ref)).join(", ")} — the lesson is withheld once one stops resolving`);
      }
      if (saved.validTo) {
  ...
  if (cmd === "promote") {
    const id = rest[1];
    if (!id) {
      console.error("usage: tlc harness lessons promote <id>   (copies a project lesson into the global tier)");
      process.exit(1);
    }
