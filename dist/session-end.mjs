...
function claudeLessonsSourcePath(root) {
  return join23(dirname6(projectConfigPath(root)), "lessons.md");
}
function claudeMdPath(root) {
  return join23(root, "CLAUDE.md");
}
function renderClaudeLessonsView(root) {
  if (!existsSync21(claudeLessonsSourcePath(root))) {
    return null;
  }
  const path = claudeMdPath(root);
  const existing = existsSync21(path) ? readFileSync21(path, "utf8") : "";
...
import { existsSync as existsSync22, mkdirSync as mkdirSync16, readFileSync as readFileSync22, writeFileSync as writeFileSync15 } from "node:fs";
import { dirname as dirname7, join as join24 } from "node:path";
function cursorLessonsSourcePath(root) {
  return join24(dirname7(projectConfigPath(root)), "lessons.md");
}
function cursorLessonsViewPath(root) {
  return join24(root, ".cursor", "rules", "harness-lessons.mdc");
}
function renderCursorLessonsView(root) {
  ...
  const body = readFileSync22(sourcePath, "utf8");
