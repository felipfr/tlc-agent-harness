...
import { existsSync as existsSync23, mkdirSync as mkdirSync15, readFileSync as readFileSync23, writeFileSync as writeFileSync14 } from "node:fs";
...
function renderClaudeLessonsView(root) {
  if (!existsSync23(claudeLessonsSourcePath(root))) {
    return null;
  }
  const path = claudeMdPath(root);
  const existing = existsSync23(path) ? readFileSync23(path, "utf8") : "";
...
import { existsSync as existsSync24, mkdirSync as mkdirSync16, readFileSync as readFileSync24, writeFileSync as writeFileSync15 } from "node:fs";
...
function renderCursorLessonsView(root) {
  ...
  const body = readFileSync24(sourcePath, "utf8");
