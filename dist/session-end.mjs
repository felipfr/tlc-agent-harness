...
function claudeLessonsSourcePath(root) {
  return join18(dirname6(projectConfigPath(root)), "lessons.md");
}
function claudeMdPath(root) {
  return join18(root, "CLAUDE.md");
}
function renderClaudeLessonsView(root) {
  if (!existsSync17(claudeLessonsSourcePath(root))) {
    return null;
  }
  const path = claudeMdPath(root);
  const existing = existsSync17(path) ? readFileSync18(path, "utf8") : "";
...
function cursorLessonsSourcePath(root) {
  return join19(dirname7(projectConfigPath(root)), "lessons.md");
}
function cursorLessonsViewPath(root) {
  return join19(root, ".cursor", "rules", "harness-lessons.mdc");
}
