...
import { existsSync as existsSync20, mkdirSync as mkdirSync14, readFileSync as readFileSync20, writeFileSync as writeFileSync13 } from "node:fs";
...
function claudeLessonsSourcePath(root) {
  return join21(dirname6(projectConfigPath(root)), "lessons.md");
}
function claudeMdPath(root) {
  return join21(root, "CLAUDE.md");
}
function renderClaudeLessonsView(root) {
  if (!existsSync20(claudeLessonsSourcePath(root))) {
    return null;
  }
  const path = claudeMdPath(root);
  const existing = existsSync20(path) ? readFileSync20(path, "utf8") : "";
...
import { existsSync as existsSync21, mkdirSync as mkdirSync15, readFileSync as readFileSync21, writeFileSync as writeFileSync14 } from "node:fs";
import { dirname as dirname7, join as join22 } from "node:path";
function cursorLessonsSourcePath(root) {
  return join22(dirname7(projectConfigPath(root)), "lessons.md");
}
function cursorLessonsViewPath(root) {
  return join22(root, ".cursor", "rules", "harness-lessons.mdc");
}
function renderCursorLessonsView(root) {
  ...
  const body = readFileSync21(sourcePath, "utf8");
...
// src/entrypoints/session-end.ts
...
var sessionEndHandler = async (event, ctx) => {
  const { policy } = ctx;
