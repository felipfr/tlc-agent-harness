...
import { existsSync as existsSync19, mkdirSync as mkdirSync14, readFileSync as readFileSync19, writeFileSync as writeFileSync13 } from "node:fs";
...
function claudeLessonsSourcePath(root) {
  return join20(dirname6(projectConfigPath(root)), "lessons.md");
}
function claudeMdPath(root) {
  return join20(root, "CLAUDE.md");
}
function renderClaudeLessonsView(root) {
  if (!existsSync19(claudeLessonsSourcePath(root))) {
    return null;
  }
  const path = claudeMdPath(root);
  const existing = existsSync19(path) ? readFileSync19(path, "utf8") : "";
...
import { existsSync as existsSync20, mkdirSync as mkdirSync15, readFileSync as readFileSync20, writeFileSync as writeFileSync14 } from "node:fs";
import { dirname as dirname7, join as join21 } from "node:path";
function cursorLessonsSourcePath(root) {
  return join21(dirname7(projectConfigPath(root)), "lessons.md");
}
function cursorLessonsViewPath(root) {
  return join21(root, ".cursor", "rules", "harness-lessons.mdc");
}
function renderCursorLessonsView(root) {
  ...
  const body = readFileSync20(sourcePath, "utf8");
...
function claudeSettingsPath() {
  return join22(claudeConfigDir(), "settings.json");
}
...
function cursorWiring(runtime) {
  ...
  return {
    target: join23(cursorConfigDir(), "hooks.json"),
    strategy: "replace",
    entries
  };
}
...
function recordAdapterEvent(root, kind, attrs) {
  try {
    appendRecord(join24(projectStateDir(root), "obs.jsonl"), {
      schema: "harness.observability.v1",
      provider: "unknown",
      kind,
      level: "signal",
