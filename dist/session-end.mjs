...
import { existsSync as existsSync18, mkdirSync as mkdirSync13, readFileSync as readFileSync18, writeFileSync as writeFileSync12 } from "node:fs";
...
function claudeLessonsSourcePath(root) {
  return join19(dirname6(projectConfigPath(root)), "lessons.md");
}
function claudeMdPath(root) {
  return join19(root, "CLAUDE.md");
}
function renderClaudeLessonsView(root) {
  if (!existsSync18(claudeLessonsSourcePath(root))) {
    return null;
  }
  const path = claudeMdPath(root);
  const existing = existsSync18(path) ? readFileSync18(path, "utf8") : "";
...
` : `${IMPORT_LINE}
`;
  mkdirSync13(dirname6(path), { recursive: true });
...
function cursorLessonsSourcePath(root) {
  return join20(dirname7(projectConfigPath(root)), "lessons.md");
}
function cursorLessonsViewPath(root) {
  return join20(root, ".cursor", "rules", "harness-lessons.mdc");
}
function renderCursorLessonsView(root) {
  ...
  const body = readFileSync19(sourcePath, "utf8");
