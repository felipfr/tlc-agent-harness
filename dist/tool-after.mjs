...
function overridesPath() {
  return join23(runtimeHome(), "model-prices.json");
}
function providerNativePath(provider) {
  return join23(runtimeHome(), `model-prices.${provider}.json`);
}
function litellmPath() {
  return join23(runtimeHome(), "model-prices.litellm.json");
}
function aliasesPath() {
  return join23(runtimeHome(), "model-aliases.json");
}
...
function claudeSettingsPath() {
  return join24(claudeConfigDir(), "settings.json");
}
...
function cursorWiring(runtime) {
  ...
  return {
    target: join25(cursorConfigDir(), "hooks.json"),
    strategy: "replace",
    entries
  };
}
...
function recordAdapterEvent(root, kind, attrs) {
  try {
    appendRecord(join26(projectStateDir(root), "obs.jsonl"), {
      schema: "harness.observability.v1",
      provider: "unknown",
      kind,
      level: "signal",
