...
// tools/doctor.ts
import { existsSync as existsSync19, lstatSync, readFileSync as readFileSync20, readlinkSync } from "node:fs";
...
function isCursorWired(targetPath) {
  return existsSync3(targetPath) && readFileSync3(targetPath, "utf8").includes(CURSOR_MARKER);
}
if (false) {}
...
async function writeJsonAtomic(path, value, options = {}) {
  const {
    ...
    sleep,
    rename = renameSync,
    writeFile = (p, data) => writeFileSync3(p, data, "utf8"),
    removeFile = (p) => {
      try {
        rmSync(p, { force: true });
      } catch {}
...
async function withFileLock(lockPath, fn) {
  mkdirSync4(dirname4(lockPath), { recursive: true });
  const attempts = 200;
  let acquired = false;
  for (let attempt = 0;attempt < attempts; attempt++) {
    try {
...
async function listAddedLines(projectDir, relativePaths) {
  if (!existsSync6(join6(projectDir, ".git")) || relativePaths.length === 0) {
    return [];
  }
  ...
  for (const file of relativePaths) {
    if (!tracked.has(file)) {
      let raw = "";
      try {
        raw = readFileSync6(join6(projectDir, file), "utf8");
      } catch {
        continue;
      }
      raw.split(/\r?\n/).forEach((text, index) => {
...
function diskLineReader(projectDir) {
  ...
  return (file, line) => {
    ...
    if (lines === undefined) {
      try {
        lines = readFileSync7(join7(projectDir, file), "utf8").split(`
`);
      } catch {
        lines = [];
      }
...
import { existsSync as existsSync7, mkdirSync as mkdirSync5, readFileSync as readFileSync8, unlinkSync, writeFileSync as writeFileSync4 } from "node:fs";
...
function lastGatePath(root) {
  return join8(projectStateDir(root), "last-gate.json");
}
...
function writeLastGate(args) {
  ...
  writeFileSync4(path, `${JSON.stringify(artifact, null, 2)}
`, "utf8");
  return artifact;
}
...
function tryAcquire(path, body) {
  ...
  try {
    ...
    try {
      writeFileSync5(fd, JSON.stringify(body));
    } finally {
      closeSync2(fd);
    }
    return true;
...
function handoffPath(root) {
  return join10(projectStateDir(root), "handoff.json");
}
...
function lessonsStorePath(root) {
  return join11(projectStateDir(root), "lessons.json");
}
...
function lessonsMarkdownPath(root) {
  return join12(dirname7(projectConfigPath(root)), "lessons.md");
}
...
import { existsSync as existsSync11, mkdirSync as mkdirSync8, readdirSync, readFileSync as readFileSync12, unlinkSync as unlinkSync3, writeFileSync as writeFileSync7 } from "node:fs";
...
function appendObsRecord(root, file, event) {
  ...
  try {
    appendRecord(join13(projectStateDir(root), file), event);
    return true;
  } catch {
    return false;
  }
...
function appendAuditRecord(root, record) {
  ...
  try {
    appendRecord(join13(projectStateDir(root), "audit.jsonl"), record);
    return true;
  } catch {
    return false;
  }
...
function readSignalEvents(root, file, limit = 200) {
  try {
    return readTail(join13(projectStateDir(root), file), limit);
  } catch {
    return [];
  }
}
function rollupPath(root, sessionKey) {
  return join13(projectStateDir(root), "sessions", `${sanitizeSegment(sessionKey)}.json`);
}
...
function saveRollup(root, rollup) {
  const dir = join13(projectStateDir(root), "sessions");
  if (!safeMkdir(dir)) {
    return false;
  }
  rollup.updated_at = new Date().toISOString();
  try {
    writeFileSync7(rollupPath(root, rollup.session_id), `${JSON.stringify(rollup, null, 2)}
`, "utf8");
    return true;
  } catch {
    return false;
...
function flagExists(root, flagName) {
  return existsSync12(join14(flagsDir(root), flagName));
}
function resolveMode(root, configured) {
  const modeFile = join14(projectStateDir(root), "harness-mode");
  if (existsSync12(modeFile)) {
    const raw = readFileSync13(modeFile, "utf8").trim().toLowerCase();
...
import { existsSync as existsSync13, mkdirSync as mkdirSync9, readdirSync as readdirSync2, readFileSync as readFileSync14, rmSync as rmSync2, writeFileSync as writeFileSync8 } from "node:fs";
...
function listPresenceRecords(root) {
  ...
  for (const entry of readdirSync2(dir)) {
    ...
    try {
      records.push(JSON.parse(readFileSync14(join15(dir, entry), "utf8")));
    } catch {}
  }
  return records;
}
...
import { existsSync as existsSync14, mkdirSync as mkdirSync10, readFileSync as readFileSync15, writeFileSync as writeFileSync9 } from "node:fs";
...
import { existsSync as existsSync15, readdirSync as readdirSync3, readFileSync as readFileSync16, statSync as statSync2 } from "node:fs";
...
function writeStore2(root, store) {
  try {
    ...
    writeFileSync10(storePath2(root), `${JSON.stringify(store, null, 2)}
`, "utf8");
  } catch {}
}
...
function checkRuntimePaths(home, platform) {
  const launcher = join21(home, "bin", "tlc-exec.mjs");
  const distSample = join21(home, "dist", "stop.mjs");
  const cliLink = join21(homedir3(), ".local", "bin", platform === "win32" ? "tlc.cmd" : "tlc");
  return [
    { level: "ok", name: "platform", detail: platform },
    { level: existsSync19(launcher) ? "ok" : "fail", name: "global runtime", detail: home },
    {
      level: existsSync19(distSample) ? "ok" : "fail",
      name: "dist bundles",
      detail: existsSync19(distSample) ? join21(home, "dist") : "missing — run: tlc harness build"
    },
    { level: existsSync19(launcher) ? "ok" : "fail", name: "portable launcher", detail: launcher },
    {
      level: existsSync19(cliLink) || existsSync19(join21(home, "bin", platform === "win32" ? "tlc.cmd" : "tlc")) ? "ok" : "fail",
      name: "CLI on PATH",
      detail: cliLink
    }
  ];
...
function providerWiringStatus(wiring) {
  if (!existsSync19(dirname8(wiring.target))) {
    return "not-installed";
  }
  if (wiring.strategy === "replace") {
    return isCursorWired(wiring.target) ? "wired" : "detected-but-unwired";
  }
  const existingText = existsSync19(wiring.target) ? readFileSync20(wiring.target, "utf8") : null;
...
function checkProviders(registry, home) {
  const launcherPath = join21(home, "bin", "tlc-exec.mjs");
  ...
  return [
    {
      level: "ok",
      name: "project policy",
      detail: existsSync19(configPath) ? configPath : "missing — run: tlc harness init"
    },
    {
      level: "ok",
      name: "state dir",
      detail: existsSync19(stateDir) ? stateDir : `${stateDir} (created on first session)`
    }
  ];
}
function checkGlobalCommands(home) {
  const globalCommands = join21(home, ".cursor", "commands");
  if (!existsSync19(globalCommands)) {
    return {
      level: "ok",
