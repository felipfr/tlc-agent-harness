...
// src/entrypoints/tool-before.ts
var READONLY_BLOCKED_TOOLS = new Set(["Write", "Delete", "Shell"]);
function recordShellDecision(event, ctx, decision) {
...
function handleShellBefore(event, ctx) {
  const { policy } = ctx;
...
async function handleToolBefore(event, ctx) {
  const { policy, provider } = ctx;
  const isReadOnlySubagent = event.subagentType !== undefined && policy.subagents.readOnlyTypes.includes(event.subagentType);
...
var toolBeforeHandler = (event, ctx) => {
