...
The measured record, each instance its own decision or fix: `attrs.permission` (AD-026), `gate.outcome` and
`policy.deny` (AD-027), `observe.rails` values (AD-029), `format.enabled` (AD-033), `LessonSource "manual"`
(AD-035), and — found by this check on its first run — `FailureCategory "infra"` and the whole `GenAiOutcome`
union.
