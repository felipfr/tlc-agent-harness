...
The mechanism is taken from Spektor's `StaleDetector`, which stores `linkedSymbolsJson` on a memory and marks the
...
conservative direction. Dropped: the LSP-backed symbol checker. Spektor's own fallback is
...
switch undoes it. Only an expired validity window — which the author declared — is pruned (AD-037).
