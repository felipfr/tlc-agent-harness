...
capability, because `tlc harness` mutations are the operator's from a terminal (AD-022) and the init skill is the
...
- **Surfacing the off state in `doctor`.** `checkLessonHealth` is silent when the capability is off, deliberately —
  a row about a disabled feature is noise on every healthy run (AD-034). The synced file is the place an operator
