...
migration: "Run `tlc harness update` once to record where this project stands. The first run announces nothing by design; the next one lists what landed and what needs action."
...
3. **`migration` is what marks a decision as needing operator action.** Nothing finer, because anything finer would
   be the harness guessing whether a given config is affected — and `doctor` answers that precisely.
...
- **Migration notes that the harness applies.** `config.json` is a protected surface and policy is the operator's
  (AD-022). A note says what to run; running it is not the harness's call.
