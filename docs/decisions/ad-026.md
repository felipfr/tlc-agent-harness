...
1. **The `paired` ask marks a command that can lose something, not one that changes something.** The shell
   classifier gains `write-preserving` between `read` and `write`: metadata changes (`chmod`, `chown`) and
   appends (`>>`) leave the asking tier. `cp`, `mv` and `tee` stay in it, because each can overwrite a
   destination and the harness cannot know whether that destination exists.
...
The narrowing is modest by design, and worth stating plainly: three shapes left the asking tier. `cp` and `mv`
stayed because the destination may exist, which is unknowable without touching the filesystem, and `tee` stayed
with or without `-a` because the flagless form truncates and a rule whose failure mode is silent data loss does
not get a special case. The residual noise is exactly where a real overwrite could happen.
