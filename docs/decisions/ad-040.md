...
Spektor has the same three-layer shape, and its personal layer explicitly holds `cross_project` — "knowledge I
apply across any project" — with promotion drawn as `Personal → Team → Project`, one direction, operator-invoked.
The shape transferred; the middle layer did not. Spektor's Layer 2 is a synced team database. This harness has no
...
Spektor auto-promotes personal → team on decay and access thresholds, and that is defensible inside one project
