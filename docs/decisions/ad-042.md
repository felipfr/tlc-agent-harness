...
`obs.globalSpool` (AD-018) had already put a file under the runtime home and did not expose this, because no test
...
Exposing it on the facade is the sanctioned route (AD-004). "Presentation does not belong in core" was the right
...
By running the thing and reading the output — the practice AD-034 records, not a mechanism. `check-wiring` (AD-041)
